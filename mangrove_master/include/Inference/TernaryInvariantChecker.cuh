...
#include "Inference/MonotonyChecker.hpp"

namespace mangrove {

    template<int F_INDEX>
    struct RecursiveMatchTernaryRanges {
        ...
        inline static bool ApplyT (ResultCollector<numeric_t> & results, int left, int right_1, int right_2) {
            ...
            return RecursiveMatchTernaryRanges<F_INDEX-1>::ApplyT(results, left, right_1,  right_2);
        }
    };

    template<>
    struct RecursiveMatchTernaryRanges<0> {
